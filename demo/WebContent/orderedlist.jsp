...
<head>
 ...
 <body>
	...
	<c:forEach items="${orderedlist }" var="i">
  ...
  <nav class="navbar fixed-top navbar-expand-lg navbar-light white scrolling-navbar">
	...
	<c:choose> 
		<c:when test="${y == 1}">
						<h4 style="margin-top: 40px;">You ordered(<c:out value="${y}"/> item)</h4>
		</c:when>
		<c:when test="${y > 1}">
						<h4 style="margin-top: 40px;">You ordered(<c:out value="${y}"/> items)</h4>
		</c:when>
		<c:otherwise >
				<h4 style="margin-top: 40px;">You have not ordered any product</h4>
		</c:otherwise>
		...
		<c:forEach items="${orderedlist }" var="i">
			<c:forEach items="${list }" var="Product">
				...
				<tr>
					<td style="width: 100px;border: 1px solid black;padding:5px;"><img src="${Product.getImage()}" height="100" width="150" >  (<c:out value="${Product.getName()}"/>)</td>
					<td style="width: 50px;border: 1px solid black;padding:5px;"><c:out value="${Product.getPrice()}"/></td>
					<td style="width: 100px;border: 1px solid black;padding:5px;"><c:out value="${Product.getCategory()}"/></td>
				</tr>
	...
	<h4 style="margin-top: 40px;margin-bottom: 40px;">Order Total: (<c:out value="${ total}"></c:out>) VND</h4>
